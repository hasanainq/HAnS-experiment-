...
public class Controller {
    ...
    public void mainMenu() throws Exception {
        char choice = Tools.getChar("Main Menu: \n " +
                ...
                "1. Enter m,M for Manager \n" +
                "2. Enter e.E for Employee \n" +
                "3. Enter c,C for Customer \n" +
                "4. Enter x,X to exit system \n");
        if(Tools.validateChar(choice, "MECXmecx"))
            switch (choice) {
                case 'm','M' -> {
                    boolean verified = Tools.verifyPassword("admin1234");
                    if(verified){
                        managerLoggedIn();
                    }
                    else System.out.println("Manager password is incorrect");
                }
                case 'e','E' -> {
                    boolean verified = Tools.verifyPassword("password123");
                    if(verified){
                    ...
                    } else System.out.println("Employee password is incorrect");
                    if(verified) System.out.println("Employee with entered name does not exist");
                }
                case 'c','C' -> {
                    ...
                    if(!found) System.out.println("Customer with entered ID not found");
                }
        ...
        do {
            ...
            if(Tools.validateChar(choice, screens))
                switch (choice) {
                    ...
                    case '3' -> System.out.println("Successfully registered customer: " + employee.registerCustomer(customerList, Tools.getString("Enter the customer's name: ", input), Tools.getPassword(), input));
                    case '4' -> {
    ...
    public void customerLoggedIn(Customer customer) throws Exception {
        char choice;
        do {
